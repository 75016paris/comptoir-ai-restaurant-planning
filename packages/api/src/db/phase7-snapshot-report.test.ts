...
import { createPhase7CoreSnapshot } from "./phase7-core-snapshot";
...
function createSourceDb() {
  ...
  `);
  db.exec(`
    INSERT INTO owners (id, name) VALUES ('owner-a', 'Owner A'), ('owner-b', 'Owner B');
    ...
    INSERT INTO open_shifts (id, restaurant_id, created_by, date, start_time, end_time, role, required_sub_roles, candidate_ids, rejected_candidate_ids, solicited_candidate_ids, status, claimed_by, service_id, expires_at, created_at) VALUES ('open-a', 'resto-a', 'user-a', '2026-05-26', '18:00', '22:00', 'floor', '[]', '["user-a"]', '[]', '[]', 'claimed', 'user-a', 'service-a', '2099-01-01', 'now');
    INSERT INTO restriction_requests (id, worker_id, restaurant_id, kind, effective_from, effective_until, restrictions, status, reviewed_by, created_at) VALUES ('restriction-request-a', 'user-a', 'resto-a', 'temporary', '2026-06-01', '2026-06-07', '[]', 'approved', 'user-a', 'now');
    ...
    INSERT INTO notifications (id, recipient_id, owner_id, restaurant_id, type, channel, message, status, scheduled_for, created_at) VALUES
      ('notification-master', 'user-a', 'owner-a', NULL, 'payment_failed', 'whatsapp', 'Payment', 'queued', 'now', 'now'),
      ('notification-owner', 'user-a', 'owner-a', 'resto-a', 'schedule_change', 'whatsapp', 'Planning', 'queued', 'now', 'now');
...
describe("Phase 7 snapshot report", () => {
  test("builds a stable identity report for a verified snapshot", () => {
    const source = createSourceDb();
    const directory = join(mkdtempSync(join(tmpdir(), "comptoir-phase7-report-")), "snapshot");

    createPhase7CoreSnapshot({ source, directory });

    const report = buildPhase7SnapshotReport(directory);

    expect(report.status).toBe("pass");
    expect(report.snapshotVersion).toBe(1);
    expect(report.master.sha256).toHaveLength(64);
    expect(report.master.copied?.loginIdentities).toBe(2);
    expect(report.master.copied?.ownerLegalAcceptances).toBe(1);
    ...
    expect(report.inventory.missingExpectedFiles).toEqual([]);
    expect(report.inventory.unexpectedSqliteFiles).toEqual([]);
    expect(report.tableCoverage.ownerTablesCopied).toEqual([
      "admin_alerts",
      "audit_logs",
      "calendar_events",
      "contract_templates",
    ...
    expect(report.tableCoverage.ownerTablesRemaining).toEqual([]);
    expect(report.tableCoverage.splitTablesRemaining).toEqual([]);
    expect(report.tableCoverage.splitTablesRemaining).not.toContain("notifications");
    expect(report.tableCoverage.splitTablesRemaining).not.toContain("chat_messages");
    expect(report.tableCoverage.splitTablesRemaining).not.toContain("cron_runs");
    expect(report.tableCoverage.splitTablesRemaining).not.toContain("users");
    expect(report.tableCoverage.splitTablesRemaining).not.toContain("legal_acceptances");
    expect(report.remainingSplitRows).toEqual({});
    expect(report.splitSchemaIssues).toEqual([]);
    expect(report.splitScopeGaps).toEqual([]);
    expect(report.remainingSplitPlans).toEqual({});
    expect(report.master.copied?.notifications).toBe(1);
    expect(report.master.copied?.chatMessages).toBe(1);
    expect(report.master.copied?.cronRuns).toBe(1);
    expect(report.totals.notifications).toBe(1);
    expect(report.totals.chatMessages).toBe(1);
    expect(report.totals.cronRuns).toBe(1);
    expect(report.owners.map((owner) => owner.ownerId)).toEqual(["owner-a", "owner-b"]);
    expect(report.verificationFailures).toEqual([]);
  });

  test("reports unexpected sqlite files in the snapshot directory", () => {
    const source = createSourceDb();
    const directory = join(mkdtempSync(join(tmpdir(), "comptoir-phase7-report-extra-")), "snapshot");

    createPhase7CoreSnapshot({ source, directory });
    const extraDir = join(directory, "owners", "extra");
    mkdirSync(extraDir, { recursive: true });
    const extraFile = join(extraDir, "comptoir.sqlite");
    writeFileSync(extraFile, "");

    const report = buildPhase7SnapshotReport(directory);

    expect(report.status).toBe("fail");
    expect(report.inventory.unexpectedSqliteFiles).toEqual([extraFile]);
    expect(report.verificationFailures).toContain(`inventory: unexpected SQLite file ${extraFile}`);
  });

  test("returns a failing report when the snapshot is missing", () => {
    const directory = mkdtempSync(join(tmpdir(), "comptoir-phase7-report-missing-"));

    const report = buildPhase7SnapshotReport(directory);

    expect(report.status).toBe("fail");
    expect(report.snapshotVersion).toBe(null);
    expect(report.verificationFailures).toContain(
      `missing manifest file ${join(directory, "phase7-snapshot-manifest.json")}`,
    );
    expect(report.inventory.missingExpectedFiles).toEqual([join(directory, "master.sqlite")]);
  });
});
