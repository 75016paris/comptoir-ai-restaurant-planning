-- Beta taxonomy: drop "flexibilite" preset.
-- Reduced to 4 differentiated presets: equilibre, equipe-stable, economique, resilience.
-- Flexibilite has no working lever that differentiates it from equilibre in the v2 solver
-- (subroleMismatch + rolePenalty tuning overlap with redundancy and mode toggles).
--
-- Same rebuild pattern as 0072 — SQLite can't ALTER a CHECK constraint.

PRAGMA foreign_keys = OFF;

-- 1. Remap any current flexibilite owners back to the default baseline.
UPDATE restaurants SET preferred_style = 'equilibre'
  WHERE preferred_style = 'flexibilite';

-- 2. Rebuild the restaurants table with the tightened CHECK constraint.
--    Column list mirrors the exact shape post-migration 0072.
