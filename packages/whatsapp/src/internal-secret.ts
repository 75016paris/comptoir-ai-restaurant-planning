...
export function assertInternalRouteSecretForProduction(env: NodeJS.ProcessEnv = process.env): void {
  if (!isProductionLikeEnv(env)) return;
  if (!isUsableDemoChatSecret(env.DEMO_CHAT_SECRET)) {
    throw new Error("DEMO_CHAT_SECRET must be set to a non-default value in production/staging for WhatsApp internal routes");
  }
}
